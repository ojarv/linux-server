...
Ensitöikseen asennetaan ```SUDO``` ja lisätään asennuksessa luotu käyttäjä ```penguin``` SUDO-ryhmään sekä estetään ```root```-käyttäjällä kirjatuminen. Lisäksi laitetaan tulimuuri päälle. Lisätietona, että tähän palvelimeen ei tule olemaan suoraa avointa yhteyttä ulkoverkosta.
